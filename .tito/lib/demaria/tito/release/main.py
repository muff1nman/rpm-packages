...
class S3Releaser(Releaser):
    """
    A releaser which will use s3cmd to download the existing repo, add the
    desired packages, and upload the result again using s3cmd to the specified
    bucket.
    ...
    """
    REQUIRED_CONFIG = ['builder', 'srpm_disttag','s3cmd']
    ...
    def release(self, dry_run=False, no_build=False, scratch=False):
        ...
        if self.releaser_config.has_option(self.target, 's3cmd_args'):
            self.s3cmd_args = self.releaser_config.get(self.target, 's3cmd_args')

        s3cmd_location = self.releaser_config.get(self.target, 's3cmd')

        # Make a temp directory to sync the existing repo contents into:
        temp_dir = mkdtemp(dir=self.build_dir, prefix=self.prefix)

        self._s3cmd_from_remote(self.s3cmd_args, s3cmd_location, temp_dir)
        self._copy_files_to_temp_dir(temp_dir)
        self.process_packages(temp_dir)
        self.s3cmd_to_remote(self.s3cmd_args, temp_dir, s3cmd_location)
    ...
    def s3cmd_to_remote(self, s3cmd_args, temp_dir, s3cmd_location):
        cmd = "s3cmd sync --delete-removed %s %s/ %s" % (s3cmd_args, temp_dir, s3cmd_location)
        print(cmd)
        os.chdir(temp_dir)
        if self.dry_run:
