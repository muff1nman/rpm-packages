...
Source0:	http://github.com/%{name}/%{name}/archive/v%{xt_tag}.tar.gz
...
# Revert redundant git commit that causes configure to fail
Patch1:	bitcoinxt-0.11C-openssl-ec-h-configure.patch

# Check for existence of LibreSSL is broken, remove it
# We don't actually build with LibreSSL
Patch2: bitcoinxt-0.11C-broken-libressl-check.patch

# Dest change address patch for Lamassu Bitcoin machine
Patch3: bitcoin-0.11.0-destchange.patch

# Increase test timeouts for slower build machines
Patch4: bitcoinxt-0.11.0-test-timeout.patch

# https://github.com/bitcoinxt/bitcoinxt/issues/101
# https://github.com/bitcoinxt/bitcoinxt/issues/104
# https://github.com/bitcoinxt/bitcoinxt/pull/102
Patch5: 102.diff

# https://github.com/bitcoinxt/bitcoinxt/pull/105
Patch6: 105.diff


BuildRoot:	%(mktemp -ud %{_tmppath}/%{name}-%{version}-%{release}-XXXXXX)

BuildRequires:	qt-devel qrencode-devel miniupnpc-devel protobuf-devel
%if 0%{?rhel} >= 7
BuildRequires:	openssl-compat-bitcoin-devel >= 1.0.2a
%else
BuildRequires:	openssl-devel
%endif
BuildRequires:	desktop-file-utils autoconf automake
BuildRequires:	checkpolicy selinux-policy-devel selinux-policy-doc
BuildRequires:	boost-devel libdb4-cxx-devel curl-devel
BuildRequires:	libtool java ImageMagick

%if 0%{?rhel} >= 7
Requires:	openssl-compat-bitcoin-libs >= 1.0.2a
%endif
...
%package devel
Summary:	Peer-to-peer digital currency
Requires:	bitcoinxt-libs%{?_isa} = %{version}-%{release}
...
%endif
Requires:	bitcoinxt-utils%{_isa} = %{version}
...
%build
# Replace Bitcoin Core icon with Bitcoin XT icon
for size in 256 128 64 32 16
do
    convert -resize ${size}x${size} src/qt/res/icons/bitcoin.png share/pixmaps/bitcoin-${size}.png
    convert -resize ${size}x${size} src/qt/res/icons/bitcoin.png share/pixmaps/bitcoin-${size}.xpm
done

# Rebrand desktop menu entry
sed -i 's/Name=Bitcoin/Name=Bitcoin XT/' contrib/debian/bitcoin-qt.desktop
...
%changelog
* Wed Dec 2 2015 Michael Hampton <bitcoin@ringingliberty.com> 0.11D-1
- Update to upstream release 0.11D

* Fri Nov 6 2015 Michael Hampton <bitcoin@ringingliberty.com> 0.11C-1
- Update to upstream release 0.11C

* Wed Sep 16 2015 Michael Hampton <bitcoin@ringingliberty.com> 0.11B-1
- Don't enable profiling
- Use upstream version numbering

* Thu Aug 27 2015 Michael Hampton <bitcoin@ringingliberty.com> 0.11.0-3
- Conflict with Bitcoin Core
- Set Requires to bitcoinxt packages, not bitcoin packages
- Replace Bitcoin Core desktop icon with Bitcoin XT icon

* Fri Aug 21 2015 Michael Hampton <bitcoin@ringingliberty.com> 0.11.0-2
- Side-port to Bitcoin XT
